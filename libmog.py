...
def repo_file(repo, *path, mkdir=False):
    ...
    if repo_dir(repo, *path, mkdir=False):
...
def repo_create(path):
    ...
    #First, we make sure the path either does'nt exist or is an empty dir
...
def tag_create(repo, name, ref, create_tag_object=False):
    ...
    if create_tag_object:
        ...
        tag.kvlm[b'tagger'] = b'mog <mog@example.com>'
        #...and a tag message!
        tag.kvlm[None] = b"A tag generated by mog, which won't let you customize the message!"
...
def object_resolve(repo, name):
    ...
    if as_branch: # Did wee find a branch?
...
argsp.add_argument("--mog-type",
                   metavar="type",
                   dest="type",
                   choices=["blob", "commit", "tag", "tree"],
                   default=None,
                   help="Specify the extended type")
...
def index_read(repo):
    ...
    assert version == 2, "mog only supportsindex file version 2"
